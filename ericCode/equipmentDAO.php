...
class equipmentDAO extends DAO {
	...
	public function getEquipmentArray(){
        ...
        if($result->num_rows >= 1){
            while($row = $result->fetch_assoc()){
                $equipment = new Equipment($row['EquipmentRecordID'], $row['EquipmentTag'], $row['Name'], $row['SerialNo'], (string)$row['Category'], $row['Quantity'], $row['EquipmentProgram'], $row['LoanDuration'], $row['Status'], $row['AuthorizationReq'], $row['Year'], $row['Cost'], $row['DateInService'], (string)$row['EquipmentVisible'], $row['EquipmentDeleted'], $row['Notes'], $row['Inventoried'], $row['InventoriedDate'], $row['ImagePath']);
    ...
    public function addEquipment($equipment){
		...
		if(!$this->mysqli->connect_errno){
			...
			EquipmentDeleted, Notes, Inventoried, InventoriedDate, ImagePath) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
            $stmt = $this->mysqli->prepare($query);
			if($stmt = $this->mysqli->prepare($query)){
				$stmt->bind_param('issiisississssssss', 
